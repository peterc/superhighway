---
title: "2020 Black Friday deals for JavaScript developers"
description: "We've found the best deals for programmers and developers this Black Friday."
date: 2020-11-27
category: "🔥 Decent discounts"
subtitle: "Offers on courses, books, tools and more."
author: "chris-brandrick"
newsletter: "javascript"
og_image: "https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/ls8wslapaknbphf0wyyl.jpg"
author_link: "https://twitter.com/chrisbrandrick"
date_separator: "|"
byline_extra: "and <a href=\"https://twitter.com/cooperx86\">Peter Cooper</a>"
---

*Note: We do not receive any commission for these links. We've just collected deals together here because we didn't want to clutter up [JavaScript Weekly](https://javascriptweekly.com/) today! :-) We found these deals via [this open call](https://twitter.com/JavaScriptDaily/status/1331967953957494784) and by searching for ourselves.*

<img src="https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/ls8wslapaknbphf0wyyl.jpg" alt="" style="width:100%; margin: 20px 0px; ">

The annual shopping bonanza that Black Friday has seemingly become isn't all just getting new socks or half price Amazon Echos --- we've found a variety of offers aimed at JavaScript developers too.

Please note that we're not responsible if someone changes their prices or pulls an offer early, so check before you buy anything. But with that said, here's our list of deals on various courses, tools, and books that you may want to check out:

------------------------------------------------------------------------

### **[Eloquent JavaScript](https://nostarch.com/ejs3) *(and others)* \[33.7% Off\]**

<img src="https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/j1est5kjbk421elzbtdt.png" alt="" style="float:right; height: 150px; margin: 0px 0px 20px 40px; ">

'A Modern Introduction to Programming' by Marijn Haverbeke is just one of the excellent books available over at [No Starch Press](https://nostarch.com).

They've an extensive range of printed titles covering programming, hacking, sys admin, and more -- well worth a look. Use the code `BLACKFRIDAY20` for a discount. This applies to No Starch's books generally, by the way, and they have lots of fantastic ones worth browsing.. I have rather too many in my own collection.

[Pragmatic Bookshelf](https://pragprog.com/) has a 40% off sale as well with code `turkeysale2020` which you can use on anything, including their many [JavaScript titles.](https://pragprog.com/search/?q=javascript)

------------------------------------------------------------------------

### **[The Learnetto Full Stack Bundle](https://learnetto.com/blackfriday?utm_source=superhighway&utm_medium=web&utm_campaign=bf2020) \[\$399 --- *a 70% discount*\]**

This next deal combines a *ton* of courses and books into one complete pack. It's a bundling of several notable resources from respected experts, covering various frontend and backend topics such as React, Tailwind CSS, GraphQL, Serverless, VSCode, and more.

CEO of Lernetto Hrishi Mittal [explains why you may want to pick up this deal in this Twitter thread](https://twitter.com/hrishio/status/1331965777751265283).

------------------------------------------------------------------------

### **[The Opinionated Guide to React](https://opinionatedreact.com/) \[30% off\]**

[A late addition!](https://twitter.com/NikkitaFTW/status/1332396357638828032) :-) Sara Vieira is a developer at CodeSandbox and *The Opinionated Guide to React* is 200 pages of 'React recipes' and examples to get you up to speed with Sara's React approach to React quickly.

Important: You need to use the code `DiscountDayOfTheYear` at checkout!

------------------------------------------------------------------------

### **[Courses from Wes Bos](https://wesbos.com/courses) \[\$69 --- *was \$139*\]**

Wes is something of a course machine, over the past few years he's dropped several high quality video courses, covering things such as Gatsby, ES6, Redux and more.

For Black Friday all his courses are half price, plus [purchasing power parity](https://en.wikipedia.org/wiki/Purchasing_power_parity) means you may get them cheaper still.

------------------------------------------------------------------------

### **[Node CLI Automation Course](https://nodecli.com) \[\$99 - *was \$249*\]**

<img src="https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/o6jdedlrzlsctyghiwmy.png" alt="" style="width:100%; ">

This step-by-step course from Ahmad Awais covers how to build both Node.js and JavaScript based CLI apps, DevTools, and npm packages for automation purposes (saving you time). It features over 100 videos, 21 example projects, access to a private Discord, and you'll get lifetime updates to any future course changes.

------------------------------------------------------------------------

### **[Jamstack Handbook from Colby Fayock](https://jamstackhandbook.com) \[\$5 - *was \$10*\]**

<img src="https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/gkmywwk1ezxtgpo6qngr.png" alt="" style="float:right; height: 150px; margin: 0px 0px 20px 40px; ">

If you're even a tiny bit curious about Jamstack, then this ebook from Colby Fayock explains what the fuss is all about.

The handbook shows how to build fast, dynamic apps with JavaScript and the static web.

------------------------------------------------------------------------

### **[Vue School](https://vueschool.io/sales/blackfriday) \[40% off - annual plan is now \$114\]**

With 364 video lessons over 26 courses, you could watch a video almost every day of the year! If Vue is on your radar, we've heard good things about this collection.

------------------------------------------------------------------------

### **[Courses from Flavio Copes](https://flaviocopes.com/blackfriday/) \[\$49\]**

<img src="https://pub-3fa0e7d81e2e40048e588fb51d66fe18.r2.dev/images/xldjwgr3oc5ik3m6asko.png" alt="" style="width:100%; ">

All of Flavio's courses are on offer this week. The author/developer has courses on a variety of subjects, including React, Vue, Next.js, Svelte, and more. You can also grab all the courses in a bundle if you're feeling particularly scholarly.

### Other deals of note:

-   Tyler McGinnis and crew are offering [25% off on their UI.dev membership](https://ui.dev/blackfriday/) which gives access to all of their courses (covering things like TypeScript, Redux and React) and other benefits.
-   Michaela Greiler is offering [45% off code review workshops](https://www.michaelagreiler.com/workshops/).
-   Testing tool [Wallaby.js is available at a discount](https://wallabyjs.com/store/personal/), starting at 25% off.
-   [Pluralsight subscriptions](https://www.pluralsight.com/offer/2020/bf-cm-40-off) are 40% off.
-   Todd Motto's [Ultimate Courses also have a 40% discount](https://ultimatecourses.com) for Black Friday.
-   Checkly is a API and E2E monitoring tool offering [35% off any plan for 12 months.](https://twitter.com/tim_nolet/status/1332016327993815041)
-   *Pricing Freelance Projects* is [half off](https://gumroad.com/l/pcUlz/BF2020), today only.
-   An [API documentation course](https://apidocsfordevs.com/) is offering 40% off till the end of the month.
-   Chris Ferdinandi has 40% off on all his [Vanilla JavaScript Pocket Guides.](https://vanillajsguides.com/) Plus \"free bonus gifts!\"
-   [Mastering Nuxt](https://masteringnuxt.com/) is 35% off till the end of the month.

*Have you found a great deal worth sharing? Drop a comment on [this Twitter thread](https://twitter.com/JavaScriptDaily/status/1331967953957494784)*.
